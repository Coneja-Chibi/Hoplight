...
Each row prints its own honest result: printed with a note on what it carries, skipped when the target has no format for that kind of piece, or failed with the error. A staged character travels as a bundle: it drags its linked lorebooks along as riders, and you can drop a rider from one run without unlinking anything.
