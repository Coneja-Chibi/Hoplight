...
A persona's own editor carries no Export button, unlike a character's. To get one out as a file, right-click the piece and choose Stage for the Press, or click it on the Press left rail, pick a target platform, and run the press (see Convert a card). A staged persona always runs as its own solo row; it never bundles into a troupe the way a character and its linked lorebooks do. A target with no persona wire, Agnai for one, prints a skip row instead: Agnai has no persona format. Its structured persona lives on a character card there, not as a file of its own.
