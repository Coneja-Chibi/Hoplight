...
- **A staged character's lorebook is missing from the zip, or printed twice.** A staged character brings its linked lorebooks along automatically as a troupe; a book already riding a troupe that way is never printed a second time, even if you also staged it on its own. Use "drop from this run" on the Press to leave one out for a single run without unlinking it, and "ride again" to bring it back.
