...
2. **Ask the file what it is.** From a terminal in the project folder, `vaud inspect <file>` names the kind and format Vaude sees, or says plainly it could not be read; `vaud validate <file>` exits 0 if the file opens at all.
...
- **"We could not read this one."** That line means Vaude doesn't recognize the format, not that your file is damaged. It's looking for a card, book, or persona shape it knows: png, json, charx, lorebook, byaf. Run `vaud inspect` on the same file for the same answer from a terminal before you file it as broken.
...
- **The receipt says a card asked for deep access.** Vaude refuses that request outright and says so on the receipt; the rest of the card still imports normally.
...
- **Fields are dimmed or missing on the Workbench.** That's the lens doing its job: pick a platform tab and the fields it can't carry leave the form, dimmed or hidden depending on your setting. Click the Vaude tab to see the whole card again; nothing was deleted, only hidden from view.
...
- `vaud inspect`, `vaud validate`, and `vaud formats` answer "is this even a file we know" faster than clicking through the studio.
