...
Rule (see [[vvs-architecture-principles]] principle 3): a format is "done" only when every authored field
in its real sample survives to an editable canonical slot. Scripts/behavior count as authored content.

## Layout

`samples/<format>/<name>.<ext>` plus a `SOURCES.md` per folder recording the exact download URL + date +
