...
`docs/ROADMAP.md` M4 scope, and `wireframes/magic/table-read.html`
(intent only, not a literal UI spec).
...
This is original engine design (no VAUDEVILLE precedent to port; designed fresh from the
...
answer -> field-patch events" plus a "progress"
...
assigned to `specs/features/personas-system.md`
...
- `wireframes\magic\table-read.html` (full
  file) - primary intent source: three tempos
