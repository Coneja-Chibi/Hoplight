...
export interface PresetSamplers {
  ...
  maxTokens?: number;
  /**
   * SillyTavern's `custom_prompt_post_processing`, carrying its real value space rather than a
   * subset. The `_tools` variants are NOT the same operation as their plain twins - they keep tool
   * definitions - so folding them together would silently change what a preset does. ST's own empty
   * string for "none" normalizes to `none` on the way in; its deprecated `claude` is kept because
