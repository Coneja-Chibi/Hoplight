...
export const classifyPaste = (text: string): Classified => {
  ...
  const kept = truncated ? text.slice(0, MAX_PASTE_BYTES) : text;
  ...
  return { kind: "card", preview: `pasted text · ${size}`, lineCount, byteLength: Math.min(rawBytes, MAX_PASTE_BYTES), truncated, text: kept };
};
