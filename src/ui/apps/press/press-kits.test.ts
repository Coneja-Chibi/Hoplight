/**
 * Kit grouping: characters own their linked lorebooks; solos don't double; drops leave the run.
 */
...
describe("groupKits", () => {
  test("a staged character pulls his linked books as riders, even unstaged ones", () => {
    const g = groupKits([p("adrian", "character")], studio, { "character:adrian": ["cargo"] });
    expect(g.kits).toHaveLength(1);
    expect(g.kits[0]!.riders.map((r) => r.id)).toEqual(["cargo"]);
    expect(g.solos).toEqual([]);
  });

  test("a staged book already riding a kit is not doubled as a solo", () => {
    const g = groupKits(
  ...
  test("a dangling ref (book deleted) is skipped, never invented", () => {
    const g = groupKits([p("adrian", "character")], studio, { "character:adrian": ["gone", "cargo"] });
    expect(g.kits[0]!.riders.map((r) => r.id)).toEqual(["cargo"]);
  });
});
